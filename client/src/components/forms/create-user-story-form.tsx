...
export const CreateUserStoryForm: React.FC<CreateUserStoryFormProps> = ({values, onSubmit, buttonText, projectId}) => {
    ...
    return (
        <div>
            <Form {...form}>
                <form className="flex-col-center space-y-4" onSubmit={handleSubmit(submitHandler)}>
                    <FormGroup>
                        <FormField
                        name="name"
                        ...
                        render={({field}) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input type="text" {...field} id="name" disabled={isSubmitting} placeholder="E.g. As a user, I want to..." />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
                        ...
                        render={({field}) => (
                            <FormItem>
                                <FormLabel htmlFor="estimationPoints">Estimation Points</FormLabel>
                                <FormControl>
                                    <Input type="number" {...field} id="estimationPoints" disabled={isSubmitting} placeholder="E.g. 5" />
                                </FormControl>
                                <FormDescription>
                                    Estimation points are used to measure the complexity of a user story. The higher the number, the more complex the user story is.
                                </FormDescription>
                                <FormMessage />
                            </FormItem>
                        )}
                    ...
                    <FormGroup>
                        ...
                        render={({field}) => (
                            <FormItem>
                                <FormLabel htmlFor="acceptanceCriteria">Acceptance Criteria</FormLabel>
                                <FormControl>
                                    <Textarea
                                    {...field}
                                ...
                                <FormDescription>
                                    Separate each acceptance criteria with a new line.
                                </FormDescription>
                                <FormMessage />
                            </FormItem>
                        )}
                    ...
                    <div className="flex w-full items-center justify-end">
                        <Button asChild type="button" disabled={isSubmitting} variant={"secondary"} className="mr-4">
                            <Link to={`/projects/${projectId}`}>Cancel</Link>
                        </Button>
                        <Button type="submit" variant={"brand"} disabled={isSubmitting}>
                            {buttonText}
                            {isSubmitting && <Icons.spinner className="animate-spin ml-2 h-4 w-4" />}
                        </Button>
                    </div>
                </form>
            </Form>
